            ...
            if (neighbor->visitid) {
              ...
              FOREACHvertex2_(vertices2) {
                if (ver != qh_pointid(vertex2->point)) {
                  tmp_qh_pointid = qh_pointid(vertex2->point);
                  /* Add the qh_pointid of the adjacent vertex if it is valid;
                     see e.g. https://github.com/qhull/qhull/blob/master/src/libqhull/libqhull.h
                  if(qh_IDnone != tmp_qh_pointid &&
                     qh_IDinterior != tmp_qh_pointid &&
                     qh_IDunknown != tmp_qh_pointid)
                  */
                  if(tmp_qh_pointid >= 0){
                    adjst.adj[adjst.nadj] = (int)tmp_qh_pointid;
                    adjst.nadj ++;
                  };
#ifdef DEBUG
                  if(tmp_qh_pointid > 32767 || (int)tmp_qh_pointid > 32767){
                    printf("WARNING: vozutil: tmp_qh_pointid, (int)... = %d %d\n",
                           tmp_qh_pointid, (int)tmp_qh_pointid);
                  };
#endif
...
#ifdef DEBUG
            if((*adjs)[ver].adj[count] > 32767 || (*adjs)[ver].adj[count] < 0){
              printf("vozutil WARNING: (*adjs)[ver=%d].adj[count=%d] = \n",
                     ver, count, (*adjs)[ver].adj[count]);
            };
#endif
            count++;
          }
